...
impl DomRenderer {
    ...
    /// テキストを描画
    fn render_text(&mut self, content: &str, position: [f32; 2], size: f32, color: [f32; 4], font: &str, max_width: Option<f32>, scroll: bool, depth: f32) {
        let pos = self.apply_transform(position, scroll);
        let rgba = format!("rgba({}, {}, {}, {})", 
            (color[0] * 255.0) as u8,
            (color[1] * 255.0) as u8,
                        ...
                        if let Ok(element) = document.create_element("div") {
                            if let Ok(element) = element.dyn_into::<HtmlElement>() {
                                ...
                                if let Some(width) = max_width {
                                    let _ = style.set_property("max-width", &format!("{}px", width * self.scale_factor));
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            let max_width_style = if let Some(w) = max_width {
                format!(" max-width: {}px; overflow-wrap: break-word;", w * self.scale_factor)
            } else {
                String::new()
            };
            ...
            let html = format!(
                r#"<div style="position: absolute; left: {}px; top: {}px; font-size: {}px; color: {}; font-family: {}; white-space: pre-wrap;{} z-index: {};">{}</div>"#,
                pos[0], pos[1], size * self.scale_factor, rgba, font, max_width_style, (1000.0 * (1.0 - depth)) as i32, escaped_content
            );
            self.html_buffer.push(html);
        }
    }
    ...
    /// スクロールとスケールファクターを適用した座標変換
    fn apply_transform(&self, position: [f32; 2], apply_scroll: bool) -> [f32; 2] {
        let mut pos = [position[0] * self.scale_factor, position[1] * self.scale_factor];
        if apply_scroll {
            pos[0] += self.scroll_offset[0];
            pos[1] += self.scroll_offset[1];
        }
        pos
    }

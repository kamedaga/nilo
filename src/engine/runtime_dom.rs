    ...
    S: super::state::StateAccess + 'static + Clone + std::fmt::Debug,
{
    ...
    let get_window_size = || -> [f32; 2] {
        if let Some(window_obj) = window() {
            let width = window_obj.inner_width().ok()
                .and_then(|v| v.as_f64())
                .unwrap_or(800.0) as f32;
            let height = window_obj.inner_height().ok()
                .and_then(|v| v.as_f64())
    ...
    // 初期レンダリング
    {
        ...
        let scroll_offset_val = *scroll_offset.lock().unwrap();
    ...
    if let Some(window_obj) = window() {
        if let Some(document) = window_obj.document() {
            if let Some(body) = document.body() {
                ...
                body.add_event_listener_with_callback("mousemove", closure.as_ref().unchecked_ref()).ok();
                closure.forget();
                ...
                let closure = Closure::wrap(Box::new(move |_event: web_sys::MouseEvent| {
                    *mouse_down_clone.lock().unwrap() = true;
                    ...
                    for (id, button_pos, button_size) in &state_guard.all_buttons {
                        ...
                        if in_bounds {
                            event_queue_clone.lock().unwrap().push(UIEvent::ButtonPressed { id: id.clone() });
                            break;
                        }
                    }
                }) as Box<dyn FnMut(_)>);
                body.add_event_listener_with_callback("mousedown", closure.as_ref().unchecked_ref()).ok();
                closure.forget();

...
impl WasmRouter {
    ...
    pub fn navigate_to_timeline(&mut self, timeline: &str, params: HashMap<String, String>) -> Result<(), String> {
        ...
        if let Some(route_info) = self.routes.get(timeline) {
            ...
            log::warn!("Timeline '{}' has no URL mapping, using native navigation", timeline);
            Ok(())
        }
    }
...
impl NativeRouter {
    ...
    pub fn navigate_to_timeline(&mut self, timeline: &str, _params: HashMap<String, String>) -> Result<(), String> {
...
impl Router {
    ...
    pub fn from_app(app: &App) -> Self {
        ...
        let start_timeline = app.timelines.first()
            .map(|t| t.name.clone())
